##############################################################
#                                                            #
#           Process the Quality Control report               #
#                                                            #
##############################################################
...
for (i in 1:length(all_files)) {
  ...
  hisat2_bracket <- regmatches(select_hisat2_line, gregexpr("(?=\\().*?(?<=\\))", select_hisat2_line, perl = TRUE))
...
fastp_meta <- c(rep("Read1 before filtering", 4), rep("Read2 before filtering", 4), 
        rep("Read1 after filtering", 4), rep("Read2 after filtering", 4), rep("Filtering result", 8))
...
hisat2_result$hisat2_result <- c("total reads", "paired reads,of these:", "aligned concordantly 0 times", 
            "aligned concordantly exactly 1 time",
                                 "aligned concordantly >1 times", "pairs aligned concordantly 0 times; of these:", 
                                 "aligned discordantly 1 time", "pairs aligned 0 times concordantly or discordantly; of these:",
                                 "mates make up the pairs; of these:", "aligned 0 times", "aligned exactly 1 time", 
                                 "aligned >1 times",
                                 "overall alignment rate")
hisat2_result <- hisat2_result[, seq(dim(hisat2_result)[2], 1)]
colnames(hisat2_result)[1] <- c("hisat2_category")
dir.create("result",showWarnings = FALSE)
write.csv(hisat2_result, "reads_alignment_report.csv", row.names = FALSE)
write.csv(fastp_result, "reads_quality_control_report.csv", row.names = FALSE)
